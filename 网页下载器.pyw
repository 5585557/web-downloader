...
_VERIFY = frozenset({'challenges.cloudflare.com','challenges.tls13.com',
    'recaptcha.net','www.recaptcha.net','www.google.com','recaptcha.google.com',
    'hcaptcha.com','www.hcaptcha.com','assets.hcaptcha.com',
    'geetest.com','static.geetest.com','api.geetest.com','dynamic.geetest.com',
    'www.gstatic.com','cap.4399.com','captchas.4399.com'})
def is_verify(url):
...
def guess_ct(url, rct):
    ...
    m = {'jpg':'image/jpeg','jpeg':'image/jpeg','png':'image/png','gif':'image/gif','svg':'image/svg+xml',
         'webp':'image/webp','ico':'image/x-icon','css':'text/css','js':'application/javascript',
         'woff':'font/woff','woff2':'font/woff2','ttf':'font/ttf','otf':'font/otf',
         'mp4':'video/mp4','webm':'video/webm','mp3':'audio/mpeg','swf':'application/x-shockwave-flash',
         'json':'application/json','xml':'application/xml','html':'text/html','wasm':'application/wasm'}
    return m.get(e, rct or 'application/octet-stream')

def ext_from_ct(ct):
    if not ct: return ''
    m = {'image/jpeg':'.jpg','image/png':'.png','image/gif':'.gif','image/svg+xml':'.svg','image/webp':'.webp',
         'image/x-icon':'.ico','text/css':'.css','application/javascript':'.js','font/woff':'.woff',
         'font/woff2':'.woff2','font/ttf':'.ttf','font/otf':'.otf','application/x-shockwave-flash':'.swf',
         'video/mp4':'.mp4','video/webm':'.webm','audio/mpeg':'.mp3','application/json':'.json',
         'application/xml':'.xml','text/html':'.html','application/octet-stream':'.bin'}
    for k, v in m.items():
        if k in ct: return v
    return ''

CT_MAP = {'html':'text/html; charset=utf-8','htm':'text/html; charset=utf-8',
    'css':'text/css; charset=utf-8','js':'application/javascript; charset=utf-8',
    'png':'image/png','jpg':'image/jpeg','gif':'image/gif','svg':'image/svg+xml','webp':'image/webp',
    'ico':'image/x-icon','swf':'application/x-shockwave-flash','woff':'font/woff','woff2':'font/woff2',
    'ttf':'font/ttf','mp4':'video/mp4','webm':'video/webm','json':'application/json','xml':'application/xml'}
...
# ★ 默认首页文件名，按优先级排序
_DEFAULT_INDEX = ('index.html', 'index.htm', 'default.html', 'default.htm',
...
def _find_any_html(directory):
    """在目录中查找任意HTML文件（排除.raw）"""
    for r, d, fs in os.walk(directory):
        for fn in fs:
            if fn.lower().endswith(('.html', '.htm')) and not fn.endswith('.raw'):
                return os.path.join(r, fn)
    return None

# ==================== Hosts ====================
def _hosts_path():
    return r'C:\Windows\System32\drivers\etc\hosts' if sys.platform == 'win32' else '/etc/hosts'
def _inject(domain):
    p = _hosts_path()
    try:
...
# ==================== 翻译引擎 ====================
LANGS = {'自动检测':'auto','中文(简体)':'zh-CN','中文(繁体)':'zh-TW','英语':'en','日语':'ja','韩语':'ko',
         '法语':'fr','德语':'de','西班牙语':'es','俄语':'ru','葡萄牙语':'pt','意大利语':'it','阿拉伯语':'ar',
         '泰语':'th','越南语':'vi','印尼语':'id','马来语':'ms','印地语':'hi','荷兰语':'nl','波兰语':'pl','土耳其语':'tr'}

_LANG_CONV = {
    'google':  {'auto':'auto','zh-CN':'zh-CN','zh-TW':'zh-TW'},
    'mymemory':{'auto':'autodetect','zh-CN':'zh','zh-TW':'zh-TW'},
    'argos':  {'auto':'auto','zh-CN':'zh','zh-TW':'zt'},
    'lingva': {'auto':'auto','zh-CN':'zh','zh-TW':'zh-TW'},
}
    ...
    for _ in range(3):
        try:
            r = requests.get("https://translate.googleapis.com/translate_a/single",
                params={'client':'gtx','sl':s,'tl':t,'dt':'t','q':text}, timeout=15)
...
def _mymemory(text, tgt='zh-CN', src='auto'):
    ...
    r = requests.get("https://api.mymemory.translated.net/get",
        params={'q':text,'langpair':f'{s}|{t}'}, timeout=15)
...
def _argos(text, tgt='zh-CN', src='auto'):
    ...
    r = requests.post("https://translate.argosopentech.com/translate",
        data={'text':text,'source':s,'target':t}, timeout=15)
...
AI_PRESETS = {
    '自定义':      {'ep':'','mdl':'','hint':''},
    'DeepSeek':    {'ep':'https://api.deepseek.com/chat/completions','mdl':'deepseek-chat','hint':'注册送额度，极便宜'},
    '通义千问':     {'ep':'https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions','mdl':'qwen-turbo','hint':'注册送额度'},
    '智谱GLM':     {'ep':'https://open.bigmodel.cn/api/paas/v4/chat/completions','mdl':'glm-4-flash','hint':'注册送额度'},
    'Groq':        {'ep':'https://api.groq.com/openai/v1/chat/completions','mdl':'llama-3.3-70b-versatile','hint':'免费额度，速度快'},
    'SiliconFlow':  {'ep':'https://api.siliconflow.cn/v1/chat/completions','mdl':'Qwen/Qwen2.5-7B-Instruct','hint':'注册送额度'},
    '零一万物':     {'ep':'https://api.lingyiwanwu.com/v1/chat/completions','mdl':'yi-lightning','hint':'注册送额度'},
    'OpenAI':      {'ep':'https://api.openai.com/v1/chat/completions','mdl':'gpt-4o-mini','hint':'官方付费'},
}

def _ai(text, tgt, src, ep, key, mdl):
    h = {'Authorization':f'Bearer {key}','Content-Type':'application/json'}
    s = '自动检测' if src=='auto' else src
    r = requests.post(ep, headers=h, json={'model':mdl,'messages':[
        {'role':'system','content':f'你是翻译器。{s}→{tgt}。只输出结果，保持格式换行。'},
        {'role':'user','content':text}],'temperature':0.1}, timeout=60)
...
def _trans_html(html, sl, tl, engine, prog=None, acfg=None):
    ...
    TT = {'p','span','div','h1','h2','h3','h4','h5','h6','li','td','th','label',
          'button','a','figcaption','blockquote','caption','summary','dt','dd',
          'b','i','em','strong','small','mark','section','article','header','footer','nav','main','aside'}
    items = []
    tt = soup.find('title')
    ...
    for t in soup.find_all(True):
        ...
        for ch in list(t.children):
            if isinstance(ch, NavigableString):
                tx = ch.strip()
                if tx and len(tx)>1 and not re.match(r'^[\d\s\.\,\-\+\:\;\/\\\|\*\(\)\[\]\{\}<>@#$%^&_=]+$', tx):
                    items.append(('t',ch,tx))
...
def make_proxy(raw_path, orig_url):
    ...
    TAGS={'img':['src','data-src','data-original','data-lazy-src','data-srcset'],'link':['href'],
          'script':['src'],'video':['src','poster'],'audio':['src'],'source':['src'],'embed':['src'],
          'iframe':['src'],'object':['data'],'input':['src']}
...
_HOP=frozenset({'host','connection','transfer-encoding','keep-alive','proxy-authorization',
    'proxy-authenticate','te','trailer','upgrade','content-length'})
_SKIP_RESP=_HOP|frozenset({'content-encoding'})

class ResServer(TServer):
    def __init__(self, addr, orig_url, save_dir, mode='cors', domain=''):
    ...
    def save_path_for(self,url):
        ...
        if pp.netloc and pp.netloc!=self.orig_netloc:
            rel=os.path.join('external',safe_filename(pp.netloc.replace(':','_')),d,fn)
    ...
    def save_cache(self):
        try:
            ...
            self.log(f"✗ 缓存保存失败: {e}",'ERROR'); return 0, 0
    ...
    def load_cache(self):
        ...
        try:
            ...
            self.log(f"✗ 缓存加载失败: {e}",'ERROR'); return 0, 0
...
class ResHandler(BaseHTTPRequestHandler):
    ...
    def _try_local(self, path):
        """尝试从本地提供文件，支持目录自动查找默认首页"""
        if os.path.isfile(path): return path
        if os.path.isdir(path):
            # ★ 优先级: index → default → 任意html
            p = _find_default_html(path)
            if p: return p
        # ★ 无扩展名路径: 先试 index/default, 再试加扩展名
        if not os.path.splitext(path)[1]:
            for name in _DEFAULT_INDEX:
                t = path + '.html' if not name.endswith(('.html','.htm')) else path.replace(os.path.splitext(path)[1], os.path.splitext(name)[1])
                # 精确拼: path + name的扩展名
        ...
        try:
            ...
            try:
                ...
                if srv.mode=='hosts':
                    ...
                    if ref and '127.0.0.1' in ref: rh['Referer']=ref.replace('127.0.0.1',srv.domain)
                r=srv.sess.request(method,url,headers=rh,data=body,timeout=15,allow_redirects=True)
...
def replace_paths(html_path,orig_url,rmap,save_dir):
    ...
    TAGS={'img':['src','data-src','data-original','data-lazy-src'],'link':['href'],'script':['src'],
          'video':['src','poster'],'audio':['src'],'source':['src'],'embed':['src'],'iframe':['src'],'object':['data']}
    c=[0]
    for tag,attrs in TAGS.items():
        for el in soup.find_all(tag):
            for attr in attrs:
...
class App:
    def __init__(self):
        self.root=tk.Tk(); self.root.title("网页资源下载器"); self.root.geometry("1100x720"); self.root.minsize(950,550)
    ...
    def _save_ai_cfg(self):
        try:
            with open(os.path.join(self.cfg_dir,'ai.json'),'w',encoding='utf-8') as f: json.dump(self._ai_cfg,f,ensure_ascii=False)
        except: pass
    ...
    def _scan(self):
        ...
        for n in os.listdir(self.base_dir):
            ...
            try:
                ...
                items.append({'dir':sd,'url':m.get('url',''),'status':m.get('status','html'),
                    'ok':m.get('ok',0),'fail':m.get('fail',0),'sz':m.get('sz',0),
                    'time':datetime.fromtimestamp(os.path.getmtime(sd)).strftime('%Y-%m-%d %H:%M')})
            except: continue
    ...
    def _mk_ctx(self,w,it=False):
        ...
        def cp():
            try:
                t=w.get("sel.first","sel.last") if it else w.selection_get()
                self.root.clipboard_clear(); self.root.clipboard_append(t)
            except: pass
        def ct():
            try:
                t=w.get("sel.first","sel.last") if it else w.selection_get()
                self.root.clipboard_clear(); self.root.clipboard_append(t); w.delete("sel.first","sel.last")
            except: pass
        def ps():
    ...
    # ★ 查找HTML文件：优先 index → default → 任意html（排除.raw）
    ...
    def _show_failed(self):
        ...
        tree.heading('idx',text='#',width=40,anchor='center'); tree.heading('reason',text='原因',width=120,anchor='w')
        tree.heading('url',text='URL',width=500,anchor='w')
    ...
    def _trans_dlg(self):
        ...
        dlg=tk.Toplevel(self.root); dlg.title("网页翻译器"); dlg.geometry("580x520")
        ...
        ai_f=ttk.LabelFrame(f1,text="AI 翻译配置",padding=5)
        ai_f.grid(row=4,column=0,columnspan=2,sticky='ew',pady=(5,0)); ai_f.columnconfigure(1,weight=1)
        ttk.Label(ai_f,text="预设:").grid(row=0,column=0,sticky='w')
        preset_v=tk.StringVar(value=self._ai_cfg.get('preset','DeepSeek'))
        preset_cb=ttk.Combobox(ai_f,textvariable=preset_v,values=list(AI_PRESETS.keys()),state='readonly',width=16)
        preset_cb.grid(row=0,column=1,sticky='w',pady=1)
        hint_lbl=ttk.Label(ai_f,text="",font=('',8),foreground='gray'); hint_lbl.grid(row=0,column=2,sticky='w',padx=(5,0))
        ttk.Label(ai_f,text="Endpoint:").grid(row=1,column=0,sticky='w')
        ep_v=tk.StringVar(value=self._ai_cfg['ep']); ttk.Entry(ai_f,textvariable=ep_v,width=45).grid(row=1,column=1,columnspan=2,sticky='ew',pady=1)
        ttk.Label(ai_f,text="Key:").grid(row=2,column=0,sticky='w')
        key_v=tk.StringVar(value=self._ai_cfg['key']); ttk.Entry(ai_f,textvariable=key_v,show='*',width=45).grid(row=2,column=1,columnspan=2,sticky='ew',pady=1)
        ttk.Label(ai_f,text="Model:").grid(row=3,column=0,sticky='w')
        mdl_v=tk.StringVar(value=self._ai_cfg['mdl']); ttk.Entry(ai_f,textvariable=mdl_v,width=45).grid(row=3,column=1,columnspan=2,sticky='ew',pady=1)
        def on_preset(*_):
            name=preset_v.get()
        ...
        def go():
            ...
            if eng_key=='ai':
                if not key_v.get().strip(): return messagebox.showwarning("提示","AI翻译需要填写 API Key",parent=dlg)
                self._ai_cfg={'preset':preset_v.get(),'ep':ep_v.get(),'key':key_v.get(),'mdl':mdl_v.get()}; self._save_ai_cfg()
        ...
        def _do(hf,eng,sl,tl):
            try:
                ...
                acfg={'ep':ep_v.get(),'key':key_v.get(),'mdl':mdl_v.get()} if eng=='ai' else None
                def on_p(i,tot):
                ...
                self.root.after(0,tr_lbl.config,{"text":"✓ 完成"}); self.root.after(0,self._log,f"✓ 翻译完成: {out}",'OK')
                self.root.after(0,lambda: messagebox.showinfo("完成",f"翻译已保存到:\n{out}",parent=dlg))
            except Exception as e:
                self.root.after(0,tr_lbl.config,{"text":"✗ 失败"}); self.root.after(0,self._log,f"✗ 翻译失败: {e}",'ERROR')
                self.root.after(0,lambda: messagebox.showerror("失败",str(e),parent=dlg))
            finally: self.root.after(0,lambda: [b.config(state='normal') for b in bf.winfo_children()])
        x=self.root.winfo_x()+(self.root.winfo_width()-580)//2; y=self.root.winfo_y()+(self.root.winfo_height()-520)//2
        dlg.geometry(f'+{max(0,x)}+{max(0,y)}')
    ...
    def _ui(self):
        ...
        f1=ttk.LabelFrame(rf,text="设置",padding=5); f1.pack(fill='x',padx=5,pady=(0,5))
        ...
        self.url_e.grid(row=0,column=1,sticky='ew',padx=5,pady=2); self.url_e.bind('<Return>',lambda _:self._start()); f1.columnconfigure(1,weight=1)
        ...
        self.dir_e.grid(row=1,column=1,sticky='ew',padx=5,pady=2); ttk.Button(f1,text="更改",command=self._chg_dir,width=6).grid(row=1,column=2)
        self._mk_ctx(self.url_e); self._mk_ctx(self.dir_e)
        f2=ttk.Frame(rf); f2.pack(fill='x',padx=5,pady=(0,5))
        self.repl_v=tk.BooleanVar(value=True); ttk.Checkbutton(f2,text="完成后替换路径",variable=self.repl_v).pack(side='left',padx=5)
    ...
    def _on_sel(self,e):
        it=self._get_sel()
        if it: self.url_v.set(it['url']); self.dir_v.set(self.base_dir)
    ...
    def _start_server(self, url, save_dir, raw_path, html_path, is_launch=False):
        ...
        if pp.scheme == 'http' and not pp.port:
            ...
            if _inject(dom):
                self._hosts_dom = dom
                try:
                    self.server = ResServer(('0.0.0.0',80),url,save_dir,mode='hosts',domain=dom)
                ...
                self._log("⚠️ Hosts写入失败，降级兼容模式",'WARNING'); self._hosts_dom=None
        if not ok:
            port = free_port()
            try:
                self.server = ResServer(('127.0.0.1',port),url,save_dir,mode='cors')
                self._log("🔄 [兼容代理模式]",'WARNING')
            except Exception as e:
                self._log(f"✗ 启动失败: {e}",'ERROR'); self.server = None; return None
        cnt, sz = self.server.load_cache()
        if cnt > 0:
            self._log(f"📦 已恢复缓存: {cnt} 个资源 ({fmt_size(sz)})", 'OK')
        else:
            self._log("📦 无缓存（首次打开或缓存为空）")
    ...
    def _open_proxy(self, it):
        ...
        self._clr_log(); self._reset_st(); self._log(f"代理打开: {url}")
        try:
    ...
    def _popup(self,e):
        ...
        if i>=0: self.lb.selection_clear(0,tk.END); self.lb.selection_set(i); self.lb.activate(i); self._on_sel(e)
    ...
    def _set_busy(self):
        for b in (self.btn_go,self.btn_batch,self.btn_redl,self.btn_done,self.btn_pause,self.btn_stop,self.btn_retry): b.config(state='disabled')
    ...
    def _reset_st(self):
        for l,t in [(self.lbl_ok,"✓ 成功: 0"),(self.lbl_sk,"⊙ 跳过: 0"),(self.lbl_fl,"✗ 失败: 0"),
                     (self.lbl_ac,"↓ 下载中: 0"),(self.lbl_sz,"■ 大小: 0 B"),(self.lbl_sp,"↻ 速度: 0 KB/s")]: l.config(text=t)
    ...
    def _upd_st(self):
        if self.server and not self.server.stop:
            ...
            self.lbl_sz.config(text=f"■ 大小: {fmt_size(s['sz'])}"); self.lbl_sp.config(text=f"↻ 速度: {fmt_size(int(sp))}/s")
    ...
    def _do_retry(self):
        for url,_ in self._rdata['failed']:
            try:
                r=requests.get(url,headers={'User-Agent':'Mozilla/5.0'},timeout=10); r.raise_for_status(); data=r.content
                if self.cur_dir:
                    pp=urlparse(url); path=unquote(pp.path)
                    if not path or path=='/': path='/index.html'
                    d=os.path.dirname(path).lstrip('/'); fn=safe_filename(os.path.basename(path))
            ...
            except Exception as e: self.root.after(0,self._log,f"  ✗ [重试] {url[:50]}: {e}",'ERROR')
    ...
    def _do_batch(self,urls,bl,bp):
        ...
        for i,url in enumerate(urls):
            try:
                ...
                r=requests.get(url,headers={'User-Agent':'Mozilla/5.0','Accept':'text/html'},timeout=15)
                ...
                self._save_meta(sd,{'url':fu,'status':'html','ok':0,'fail':0,'sz':0})
    ...
    def _start(self):
        ...
        self._clr_log(); self._reset_st(); self._log(f"目标: {url}"); self._set_busy()
        threading.Thread(target=self._dl_html,args=(url,bdir),daemon=True).start()

    def _dl_html(self,url,bdir):
        try:
            s=requests.Session(); s.headers.update({'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36','Accept':'text/html,*/*;q=0.8'})
            ...
            self.root.after(0,self._save_meta,sd,{'url':fu,'status':'html','ok':0,'fail':0,'sz':0})
            self.root.after(0,self._log,f"✓ HTML已保存 ({fmt_size(len(html))})"); self.root.after(0,self._log,f"  目录: {sd}")
            self.root.after(0,self._ref_list); self.root.after(0,self._prompt)
        except Exception as e: self.root.after(0,self._log,f"✗ 下载失败: {e}",'ERROR'); self.root.after(0,self._set_idle)
    ...
    def _prompt(self):
        ...
        ttk.Label(dlg,text=f"是否打开浏览器开始下载资源？{h}",font=('',10),justify='center').pack(pady=(0,6))
    ...
    def _done(self):
        ...
        if failed:
            self._log("失败列表:"); self._rdata={'failed':failed,'save_dir':self.cur_dir,'downloaded':rmap}
        ...
        self._log("─"*50)
        self._save_meta(self.cur_dir,{'url':self.cur_url,'status':'done','ok':stats.get('ok',0),'fail':stats.get('fail',0),'sz':stats.get('sz',0)})
